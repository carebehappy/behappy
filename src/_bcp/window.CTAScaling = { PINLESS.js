window.CTAScaling = {
    ...
    setupAnimations: function(gsap, ctaSection, ctaContent) {
        ...
        // Фаза 1: Быстрый скейлинг до центра
        gsap.to(ctaSection, {
            scale: 0.75,
            borderRadius: "12px",
            ease: "none",
        ...
        // Фаза 2: МЕДЛЕННАЯ анимация в центре (имитация pin)
        gsap.to(ctaSection, {
            scale: 0.68, // Чуть меньше
            opacity: 0.3,
            ease: "power1.inOut",
            scrollTrigger: {
                trigger: ctaSection,
                start: "center center",
                end: "+=300px", // ДЛИННАЯ дистанция
                scrub: 4, // ОЧЕНЬ медленно
                invalidateOnRefresh: true
            }
        });

        // Паралакс эффект для фонового паттерна
